...
export default function BoardAnalytics({
  ...
  const holeStats = useMemo(() => {
    return Array.from({length:18}, (_,i)=> {
      const vals = completed.map(p => p.scores[i]).filter((v): v is number => Number.isFinite(v as number));
      const avg = mean(vals);
      const par = grid.par[i];
      const delta = Number.isFinite(par) && Number.isFinite(avg) ? Math.round((avg - (par as number))*10)/10 : NaN;
  ...
  const pickAward = (cat: string) => awards.filter(a=>a.category===cat);
      ...
      <div className="kpi-grid">
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-sub">
            { Number.isFinite(bestGross) && <span className="badge">
              Best {(() => {
            ...
            {holeStats.map(h => {
              ...
              return (
                <div key={h.hole} className="hole-bar">
                  <div className="hole-fill" style={{ background: bg, opacity: abs ? 0.6 + (abs/3) : 0.4 }} />
                  <b>H{h.hole}</b>
                </div>
              );
            })}
        ...
        <div className="card">
          ...
          <div className="overflow-x-auto">
            <table className="table-compact">
              <thead><tr><th>順位</th><th>名前</th><th>Gross</th><th>HC</th><th>Net</th></tr></thead>
              <tbody>
                {netRank.map((r,i)=>(
                  <tr key={i}>
                    <td>{i+1}</td>
