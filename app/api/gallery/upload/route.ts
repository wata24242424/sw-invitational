...
export async function POST(req: Request) {
  try {
    if (!ENV.ENABLE_GALLERY_UPLOAD) return new NextResponse('Upload disabled', { status: 403 });
    const form = await req.formData();
    const file = form.get('file');
    if (!(file instanceof File)) return new NextResponse('No file', { status: 400 });

    const key = `${Date.now()}_${file.name}`;
    const { error } = await supabaseService.storage.from(ENV.SUPABASE_BUCKET).upload(
      key,
